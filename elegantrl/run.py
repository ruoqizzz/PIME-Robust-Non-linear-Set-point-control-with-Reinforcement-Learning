...
class Arguments:
    def __init__(self, agent=None, env=None, gpu_id=None, if_on_policy=False):
        ...
        self.learning_start = 0  # burn in time at first
        ...
        self.eval_gap = 5  # evaluate the agent per eval_gap seconds
...
def train_and_evaluate(args):
    ...
    if 'integrator_dim' in args.Modular_kwargs:
        agent.init(net_dim, state_dim, action_dim, args.Modular_kwargs['integrator_dim'], if_per)
    else:
        agent.init(net_dim, state_dim, action_dim, if_per)

    '''init: Agent SCN parameters'''
    if len(args.SCN_kwargs.keys())>0:
        agent.init_SCN(args.SCN_kwargs)
    if len(args.residual_kwargs.keys())>0:
        agent.init_residual(args.residual_kwargs)
    if len(args.Q_kwargs.keys())>0:
        agent.init_Q(args.Q_kwargs)
    if args.if_residual:
        # print(f'if_residual {args.if_residual}')
        agent.init_actor_zero()
    if args.fix_K:
        agent.fix_K()
    if args.frozen_modular_integrator:
        agent.frozen_integrator()
        print('frozen_modular_integrator!=================')
    if args.frozen_transfer:
        agent.frozen_transfer()
        print('frozen_transfer!===========================')

    if  args.load !='None':
        print('-------------------------------------------------------------------------------------------------------------')
        agent.save_load_model(args.load, if_save=False)
        print('-------------------------------------------------------------------------------------------------------------')
    ...
    # add self design in render
    test_render = args.test_render
    test_render_times = args.test_render_times
    # del args  # In order to show these hyper-parameters clearly, I put them above.


    '''start test 0 step'''
    # print('start test 0 step')
    if_reach_goal = evaluator.evaluate_act(agent)
    # logger.record('training/total_step', 0)
    logger.dump(step=0)
    ...
    if test_render is not None:
        save_path = os.path.join(cwd,f'step_0')
        os.makedirs(save_path, exist_ok=True)
        test_render(agent, save_path)

    '''start training'''
    if not if_on_policy:
        while learning_start>0 and total_step < learning_start:
            steps = agent.explore_env(env, buffer, target_step, reward_scale, gamma)
            total_step += steps
            # obj_a, obj_c = agent.update_qnet(buffer, target_step, batch_size, repeat_times)
            ...
            logger.record('training/total_step', total_step)
            logger.dump(step=total_step)
    ...
    while not ((if_break_early and if_reach_goal)
        ...
        # self design test
        if test_render is not None and total_step % test_render_times==0:
            save_path = os.path.join(cwd,f'step_{total_step}')
            os.makedirs(save_path, exist_ok=True)
            test_render(agent, save_path)
...
class Evaluator:
    def __init__(self, cwd, agent_id, eval_times1, eval_times2, eval_gap, env, device):
        ...
        self.eval_func_time = 1
        print(f"{'ID':>2}  {'Step':>8}  {'MaxR':>8} |"
              f"{'avgR':>8}  {'stdR':>8}   {'objA':>8}  {'objC':>8} |"
              f"{'avgS':>6}  {'stdS':>4}")
        self.eval_func_time = 1

    def evaluate_act(self, agent):
        act = agent.act
        if self.eval_times1==0:
            print(f"{self.agent_id:<2}  {self.total_step:8.2e}                                                                ")
            return False
        ...
        if r_avg > self.r_max:
            self.r_max = r_avg
            agent.save_load_model(self.cwd, if_save=True)


        logger.record('rollout/ep_rew_mean', r_avg)
        logger.record('rollout/ep_rew_std', r_std)
        logger.record('rollout/log_rew_max', self.r_max)
        # save init actor
        os.makedirs(os.path.join(self.cwd,'init'))
        agent.save_load_model(os.path.join(self.cwd,'init'), if_save=True)
        ...
        self.recorder.append((self.total_step, r_avg, r_std, 0., 0.)) 
    ...
    def evaluate_save(self, agent, steps, obj_a, obj_c) -> bool:
        ...
        if self.eval_func_time % self.eval_gap==0:
            rewards_steps_list = [get_episode_return(self.env, act, self.device) for _ in range(self.eval_times1)]
            r_avg, r_std, s_avg, s_std = self.get_r_avg_std_s_avg_std(rewards_steps_list)
            ...
            # print(f"eval_func_time {self.eval_func_time}")
        else:
            if_reach_goal = False
...
def get_episode_return(env, act, device) -> (float, int):
    ...
    for episode_step in range(max_step):
        ...
        # print(f"get_episode_return, action {a_tensor}")
        if if_discrete:
            a_tensor = a_tensor.argmax(dim=1)
        action = a_tensor.detach().cpu().numpy()[0]  # not need detach(), because with torch.no_grad() outside
        state, reward, done, _ = env.step(action)
        # print(f"get_episode_return state {state}")
